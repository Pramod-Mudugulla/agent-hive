...
chat.post('/messages', async (c) => {
    ...
    // 2. Persist user message and update conversation timestamp
    ...
    const result = await routeQuery(content, history, async (text) => {
        // Persist AI message on finish and update timestamp
        await prisma.conversation.update({
            where: { id: conversation.id },
            data: {
                updatedAt: new Date(),
